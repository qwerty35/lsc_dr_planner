...
namespace LibCBS
{
  ...
  struct Constraint {
    ...
    int t;      // at t
    MAPF::Node* v;    // at t
    MAPF::Node* u;    // used only for swap conflict, at t-1
    bool stay;  // stay forever from t

    Constraint(int _id, int _t, MAPF::Node* _v, MAPF::Node* _u)
        : id(_id), t(_t), v(_v), u(_u), stay(false){};
    Constraint(int _id, int _t, MAPF::Node* _v, bool _stay)
  ...
  struct MDD {
    ...
    // used
    MDDNode* createNewNode(int t, MAPF::Node* v);
